...
$this->title = (!isset($this->entity->guid)) ? 'Editing New Dashboard' : 'Editing Dashboard for '.h($this->entity->user->name);
$this->note = 'Provide dashboard details in this form.';
$_->com_pgrid->load();
?>
<form class="pf-form" method="post" id="p_muid_form" action="<?php e(pines_url('com_dash', 'manage/save')); ?>">
		...
		<div class="tab-pane" id="p_muid_tab_advanced">
			...
			<div class="pf-element pf-full-width">
				...
				<table id="p_muid_user_grid">
					...
					<tbody>
					<?php foreach($this->user_array as $cur_user) { ?>
						<tr title="<?php e($cur_user->guid); ?>">
							<td><input type="checkbox" name="users[]" value="<?php e($cur_user->guid); ?>" <?php echo ($cur_user->dashboard->guid && $cur_user->dashboard->is($this->entity)) ? 'checked="checked" ' : ''; ?>/></td>
							<td><?php e($cur_user->name); ?></td>
							<td><?php e($cur_user->username); ?></td>
						</tr>
					<?php } ?>
					</tbody>
				</table>
