...
<script type="text/javascript">
	...
	$_(function(){
		...
		adjustment_dialog.dialog({
			...
			buttons: {
				'Save': function(){
					$_.post(<?php echo json_encode(pines_url('com_hrm', 'adjustment/save')); ?>, {
						id: adjustment_id,
		...
		var cur_defaults = {
			...
			pgrid_toolbar_contents: [
				...
				{type: 'button', text: 'Edit', extra_class: 'picon picon-document-edit', double_click: true, click: function(e, row){
					...
					$("#p_muid_adjustment_dialog [name=effective_date]").val($_.unsafe(row.pgrid_get_value(1)));
					$("#p_muid_adjustment_dialog [name=name]").val($_.unsafe(row.pgrid_get_value(2)));
					$("#p_muid_adjustment_dialog [name=employee]").val($_.unsafe(row.pgrid_get_value(3)));
					$("#p_muid_adjustment_dialog [name=amount]").val($_.unsafe(row.pgrid_get_value(4)).replace('$',''));
					$("#p_muid_adjustment_dialog [name=comments]").val($_.unsafe(row.pgrid_get_value(5)));
					adjustment_dialog.dialog("open");
				}},
