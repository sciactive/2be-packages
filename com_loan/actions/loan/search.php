...
if ($loans) {
	foreach ($loans as $key => &$cur_loan) {
		...
		$json_struct = (object) array(
			'guid'					=> $cur_loan->guid,
			'id'					=> (int) $cur_loan->id,
			'customer_name'			=> h($cur_loan->name),
			'customer_guid'			=> h($cur_loan->customer->guid),
			'employee'				=> h($cur_loan->user->name),
			'employee_guid'			=> h($cur_loan->user->guid),
			'location_guid'			=> h($cur_loan->group->guid),
			'location'				=> h($cur_loan->group->name),
			'creation_date'			=> h(format_date($cur_loan->creation_date, "date_short")),
			'status'				=> h($cur_status),
			'archived'				=> h($archived),
