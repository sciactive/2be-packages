	...
	$_(function(){
		...
		var change_code = function(code) {
			$.ajax({
				...
				beforeSend: function(){
					code_status.addClass('text-info').removeClass('text-error text-success').html('<i class="fa fa-spinner fa-spin"></i> Saving...');
				},
				error: function(XMLHttpRequest, textStatus){
					code_status.addClass('text-error').removeClass('text-info text-success').html('<i class="fa fa-times"></i> Error');
				...
				success: function(data){
					...
					if (data.success) {
						code_status.addClass('text-success').removeClass('text-info text-error').html('<i class="fa fa-check"></i> Saved!');
						// Make the grid re-search.
						var link = $('<a href="javascript:void(0);">refresh grid with same search.</a>');
						link.click(function(){
							customer_search_button.click();
						});
						var div = $('<div class="alert alert-warning error-message" style="margin-top: 10px;">Information in this grid is now outdated. Click here to </div>');
						div.append(link);
						item.append(div);
					}
				}
...
<div class="pf-form" id="p_muid_form">
	<div class="item clearfix">
		...
		<div class="name row">
			<div class="col-sm-8">
				<select class="form-control col-sm-12" name="collection_code">
            ...
            <div class="code-status text-success col-sm-4"></div>
		</div>
	</div>
</div>