...
/**
 * com_paypal main class.
 *
 * Process sales through PayPal.
 *
 * @package Components\paypal
 */
class com_paypal extends component {
	/**
	...
	function payment_credit(&$array) {
		...
		switch ($array['action']) {
			case 'request':
				$module = new module('com_paypal', 'form_payment');
				...
				break;
			case 'request_cust':
				$module = new module('com_paypal', 'form_payment_cust');
				if ($array['ticket']->customer->guid) {
			...
			case 'approve':
				...
				if ($array['type'] == 'return') {
					if (
							...
							&&
							(empty($array['payment']['com_paypal_credit_info']['name_last']) || empty($array['payment']['com_paypal_credit_info']['card_number']))
						) {
			...
			case 'tender':
				...
				$transaction_name = $pines->config->com_paypal->trans_name;

				$post_values = array(
					// the API Login ID and Transaction Key must be replaced with valid values
					'x_login'			=> $pines->config->com_paypal->apilogin,
					'x_tran_key'		=> $pines->config->com_paypal->tran_key,
					'x_test_request'	=> ($pines->config->com_paypal->test_mode) ? 'TRUE' : 'FALSE',
				...
				if ($pines->config->com_paypal->merchant_type == 'retail') {
				...
				$request = curl_init($pines->config->com_paypal->post_url);
				...
				$array['payment']['com_paypal_credit_info'] = array(
				...
				$array['payment']['com_paypal_credit_info']['transaction_id'] = $response_array[6];
			...
			case 'void':
				$post_values = array(
					...
					'x_type'			=> 'VOID',
					'x_trans_id'		=> $array['payment']['com_paypal_credit_info']['transaction_id']
				);
			...
			case 'return':
				...
				$card_num = !empty($array['payment']['data']['card_number']) ? $array['payment']['data']['card_number'] : $array['payment']['com_paypal_credit_info']['card_number'];
				$exp_date = !empty($array['payment']['data']['card_exp_month']) ? $array['payment']['data']['card_exp_month'].$array['payment']['data']['card_exp_year'] : $array['payment']['com_paypal_credit_info']['card_exp_month'].$array['payment']['com_paypal_credit_info']['card_exp_year'];
				$transaction_id = $array['payment']['com_paypal_credit_info']['transaction_id'];
				$card_type = $array['payment']['com_paypal_credit_info']['card_type'];
				$transaction_name = 'RETURN: '.$pines->config->com_paypal->trans_name;
