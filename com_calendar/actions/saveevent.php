...
if (!isset($_REQUEST['employee'])) {
	$_->page->ajax(json_encode(array('result' => false, 'message' => 'Please choose an employee for this event.')));
	return;
}
if (!empty($_REQUEST['id'])) {
	$event = com_calendar_event::factory((int) $_REQUEST['id']);
	if (!isset($event->guid)) {
		$_->page->ajax(json_encode(array('result' => false, 'message' => 'The calendar was altered while editing the event.')));
		return;
	}
	if (isset($event->appointment)) {
		$_->page->ajax(json_encode(array('result' => false, 'message' => 'You cannot edit appointments.')));
		return;
	}
	if ($event->time_off) {
		$_->page->ajax(json_encode(array('result' => false, 'message' => 'You cannot edit time off.')));
		return;
	}
	if (!gatekeeper('com_calendar/managecalendar') && !($event->user->guid && $_SESSION['user']->is($event->user))) {
		$_->page->ajax(json_encode(array('result' => false, 'message' => 'You can only edit your own events.')));
		return;
	}
} else {
...
if (!isset($event->employee->guid)) {
	...
	if (!isset($location->guid)) {
		$_->page->ajax(json_encode(array('result' => false, 'message' => 'The specified location for this event does not exist.')));
...
if ($event->save()) {
	...
	$_->page->ajax(json_encode(array('result' => true, 'message' => $action.' ['.$event->title.']')));
} else {
	$_->page->ajax(json_encode(array('result' => false, 'message' => 'Error saving event. Do you have permission?')));
}
